...
### 코멘트
 ...
 <p>이 문제는 전형적인 이진탐색 문제로 1654번 랜선 자르기 문제의 답과 유사하게 나왔다. 그냥 필요한 연산자를 바꾸기만 하면 거의 다 풀린 문제이다. 나는 이진탐색에서의 초기 right값을, 나무 길이 입력받는 중에 최댓값을 구해서 넣는 식으로 했다. 근데 다른사람코드에서는 이 right 값을 미리 충분히 큰 수로 지정해놓고 나무 길이의 최댓값을 구하지 않는 방법을 보았다. 처음 봤을땐 내 방법이 틀린줄 알았다. 그래서 gpt한테 물어보니 원래 하던 방식이 일반적으로 더 효율적이라 하는데 내 코드에서 while문안의 명령어가 많기에 그런건지 아니면 그냥 그런건진 잘 모르겠다.  </p>
